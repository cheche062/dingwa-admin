angular.module('ActiveChargeController',['CommonService','ActiveService'])
       .controller('ActiveChargeCtrl',['$scope','DictService','ActiveService','DialogService',
        function($scope,DictService,ActiveService,DialogService){
            ...
            $scope.add = function(){
                ...
                if($scope.info.charge == ''){
                    DialogService.alert('请输入充电次数')
                    return false
                }
                if($scope.info.present == ''){
                    DialogService.alert('请输入赠送充电次数')
                    return false
                }
                if(!(/^\d+$/).test($scope.addContent.charge)){
                    DialogService.alert('请输入正确的充值数')
                    return false
                }
                if(!(/^\d+$/).test($scope.addContent.present)){
                    DialogService.alert('请输入正确的赠送数')
                    return false
                }
                ...
                var condition = {
                    ...
                    payment_fix:$scope.addContent.charge+','+$scope.addContent.present,
            ...
            $scope.update = function(){
                ...
                if(!(/^\d+$/).test($scope.editContent.charge)){
                    DialogService.alert('请输入正确的充电次数')
                    return false
                }
                if($scope.editContent.present == ''){
                    DialogService.alert('请输入赠送次数')
                    return false
                }
                if(!(/^\d+$/).test($scope.editContent.present)){
                    DialogService.alert('请输入正确的赠送次数')
                    return false
                }
                ...
                var condition = {
                    ...
                    payment_fix:$scope.editContent.charge+','+$scope.editContent.present,
